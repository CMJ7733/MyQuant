"""Test-time adaptation generation module for Famou 2.0.

Enhances programs through test-time augmentation (TTA) and adaptive inference.
"""
...
class TestTimeAdaptationGenerate(GenerateModule):
    """
    Generate improved programs through test-time adaptation.

    This module specializes in enhancing inference performance through
    test-time techniques. It adapts its strategy based on complexity level:

    - Simple: Basic augmentations and averaging
    - Medium: Diverse TTA strategies and ensembling
    - High: Advanced TTA with optimization and calibration

    The template asks the LLM to:
    1. Implement appropriate TTA techniques for the task type
    2. Add prediction aggregation methods
    3. Apply calibration and post-processing
    4. Balance performance gains with computational cost

    This is useful for:
    - Computer vision competitions (geometric/photometric TTA)
    - NLP tasks (back-translation, ensembling)
    - Tabular data (feature bagging, model ensembling)
    - Kaggle competitions (TTA is often key to winning)
    ...
    Args:
        ...
        include_memory: Include previously explored TTA (default: True)
        include_experience: Include verified effective TTA (default: True)

    Example:
        >>> # Simple TTA
        >>> generator = TestTimeAdaptationGenerate(complexity="simple")
        >>>
        >>> # Advanced TTA with ensembling
        >>> generator = TestTimeAdaptationGenerate(complexity="high")
    """
    ...
    def build_prompt(self, context: Context, selection: SelectionData) -> str:
        """
        Build test-time adaptation prompt.

        Uses template from famou/prompts/templates/generation/test_time_adaptation.txt
        ...
        prompt = prompt_registry.get(
            "generation/test_time_adaptation.txt",
        ...
        self.log_info(
            f"Test-time adaptation (complexity={self.complexity}): "
    ...
    def _format_memory(self, context: Context, inspiration_ids: List[str]) -> str:
        """
        Format previously explored TTA as memory.
        ...
        for i, prog in enumerate(inspirations):
            ...
            formatted.append(
                f"## Previous TTA {i + 1} (Generation {prog.generation})\n"
                f"{program_text}"
            )
    ...
    def _format_experiences(self, context: Context, experiences: List) -> str:
        """
        Format verified effective TTA experiences.
    ...
    def validate_output(self, context: Context, result: RolloutResult) -> None:
        ...
        if not result.generated_program:
            raise ValueError(
                f"{self.name}: Failed to generate program with test-time adaptation. "
