"""Data augmentation generation module for Famou 2.0.

Enhances programs through training-time data augmentation.
"""
...
class DataAugmentationGenerate(GenerateModule):
    """
    Generate improved programs through data augmentation.

    This module specializes in implementing training-time data augmentation
    to improve model generalization. It adapts its strategy based on complexity level:

    - Simple: Fundamental augmentations with moderate intensity
    - Medium: Diverse augmentation strategies and combinations
    - High: Advanced augmentation with adaptive strategies

    The template asks the LLM to:
    1. Identify appropriate augmentation techniques for the data type
    2. Implement augmentation pipeline with proper intensity
    3. Ensure augmentation only applies during training
    4. Use established libraries (albumentations, torchvision, etc.)

    This is useful for:
    - Computer vision tasks (image augmentation)
    - NLP tasks (text augmentation)
    - Tabular tasks (SMOTE, feature mixing)
    - Audio/time series tasks
    ...
    Args:
        ...
        include_memory: Include previously explored augmentation (default: True)
        include_experience: Include verified effective augmentation (default: True)

    Example:
        >>> # Simple augmentation for images
        >>> generator = DataAugmentationGenerate(complexity="simple")
        >>>
        >>> # Advanced augmentation with MixUp/CutMix
        >>> generator = DataAugmentationGenerate(complexity="high")
    """
    ...
    def build_prompt(self, context: Context, selection: SelectionData) -> str:
        """
        Build data augmentation prompt.

        Uses template from famou/prompts/templates/generation/data_augmentation.txt
        ...
        prompt = prompt_registry.get(
            "generation/data_augmentation.txt",
        ...
        self.log_info(
            f"Data augmentation (complexity={self.complexity}): "
    ...
    def _format_memory(self, context: Context, inspiration_ids: List[str]) -> str:
        """
        Format previously explored augmentation as memory.
        ...
        for i, prog in enumerate(inspirations):
            ...
            formatted.append(
                f"## Previous Augmentation {i + 1} (Generation {prog.generation})\n"
                f"{program_text}"
            )
    ...
    def _format_experiences(self, context: Context, experiences: List) -> str:
        """
        Format verified effective augmentation experiences.
    ...
    def validate_output(self, context: Context, result: RolloutResult) -> None:
        ...
        if not result.generated_program:
            raise ValueError(
                f"{self.name}: Failed to generate program with data augmentation. "
